...
def classifyFloors(floors,site_elev):
    ...
    for floor in floors:
        ...
        floor_entities+=6*"\t"+"<floor- class=\""+type+"\" name='{}'  level='{}' volumeConcrete='{}' elev=\"{}\" >{}<span class=\"floor_stats\">{}</span> </floor->\n".format(floor.Name, level, 50.0 , floor.Elevation,floor.Name, round(float(floor.Elevation),3))     
        level-=1
