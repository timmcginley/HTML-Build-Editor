...
def writeCustomHTML(model):
    ...
    len(windows)
    custom+=5*"\t"+"<building- windows=\"{}\">\n".format(len(windows))
    
    custom+= classifyWindows(windows,floors)
...
#---------------------------------------------------------------------------------------------------------
def classifyWindows(windows,floors):
    ...
    for Window in windows:
       contain = Window.ContainedInStructure
       if len(contain) > 0:
           if contain[0].RelatingStructure.Name == "Level 1":
               count1 += 1
    type1 = count1
    #custom +=3*"\t"+"<view- a1='{}'>\n".format(type)
    ...
    for Window in windows:
            contain = Window.ContainedInStructure
            if len(contain) > 0:
                if contain[0].RelatingStructure.Name == "Level 2":
                    count2 += 1
    type2 = count2
    #custom+=3*"\t"+"<view- a2='{}'>\n".format(type)
    ...
    for Window in windows:
           contain = Window.ContainedInStructure
           if len(contain) > 0:
               if contain[0].RelatingStructure.Name == "Roof":
                    count3 += 1
    type3 = count3
    
    window_list=[type1,type2,type3]
    level_list=["Level 1","Level 2","Roof"]
        # THE SPAN STUFF SHOULD BE DEALT WITH IN JS...
    for i in range(0,len(window_list)):  
        window_count+=6*"\t"+"<window- count='{}'  Level='{}'  </window->\n ".format(window_list[i],level_list[i])     
