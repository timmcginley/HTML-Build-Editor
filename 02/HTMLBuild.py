...
def classifyFloors(floors,site_elev):
    ...
    for floor in floors:
        ...
        floor_entities+=6*"\t"+"<floor- class=\""+type+"\" name='{}'  volume'{}' level='{}' elev=\"{}\" >{}<span class=\"floor_stats\">{}</span> </floor->\n".format(floor.Name, 248,  level, floor.Elevation,floor.Name, round(float(floor.Elevation),3))     
        level-=1
