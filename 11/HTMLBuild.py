...
def writeCustomHTML(model):
    ...
    # ---- ADD Walls CUSTOM ENTITY
    wall = model.by_type('IfcWall')
    # ---- ADD windows CUSTOM ENTITY
    window = model.by_type('IfcWindow')

    # ---- ADD BUILDING CUSTOM ENTITY
    custom+=5*"\t"+"<building- walls=\"{}\" windows=\"{}\" windows=\"{}\">\n".format(len(wall),len(window),len(window))
